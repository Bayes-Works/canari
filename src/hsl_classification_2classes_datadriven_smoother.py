...
class hsl_classification:
    ...
    def detect(
        ...
        while i < len(data["x"]):
            ...
            if p_a_I_Yt > self.detection_threshold:
                ...
                # log_likelihood_ll = np.sum(decay_weights * np.log(data_likelihoods_ll))
                # log_likelihood_lt = np.sum(decay_weights * np.log(data_likelihoods_lt))
    ...
    def _estimate_likelihoods_with_intervention(self, ssm: Model, level_intervention: List[float], trend_intervention: List[float], num_steps_retract: int, data):
        ...
        # y_likelihood_all = y_likelihood_all[-52*2:-1] 
        # gamma = 0.999
        # decay_weights = np.array([gamma**i for i in range(len(y_likelihood_all)-1, -1, -1)])
        # y_likelihood_all = np.array(y_likelihood_all)
        # y_likelihood_all = y_likelihood_all * decay_weights
