...
class LstmEmbedding:
    ...
    def __getitem__(self, key):
        ...
        raise KeyError(
            "Invalid key for LstmEmbedding. Use 0/'mu', 1/'var', or [:] for both."
        )
    ...
    def __setitem__(self, key, value):
        ...
        Args:
            key: 0/'mu', 1/'var', or [:].
            value: New array (for single key) or tuple[np.ndarray, np.ndarray] for [:].
        """
        if key in (slice(None), Ellipsis):
        ...
        if key in (0, "mu"):
            self.set_values(value, self.var)
            return

        if key in (1, "var"):
            self.set_values(self.mu, value)
            return
