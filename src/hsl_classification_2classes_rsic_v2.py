...
class hsl_classification:
    ...
    def detect(
        ...
        while i < len(data["x"]):
            ...
            if p_a_I_Yt > self.detection_threshold and rerun_kf is False and first_time_trigger is False:
                first_time_trigger = True
                trigger_time = self.current_time_step

            # Apply intervention to estimate data likelihood
            if first_time_trigger and rerun_kf is False:
                ...
                if len(data_likelihoods_ll) > 0 and len(data_likelihoods_lt) > 0:
                    ...
                    joint_data_hs_ll_post_n = np.array(data_likelihoods_ll) * np.array(hs_likelihoods_ll) / \
                            (np.array(data_likelihoods_ll) * np.array(hs_likelihoods_ll) + np.array(data_likelihoods_lt) * np.array(hs_likelihoods_lt))
                    joint_data_hs_lt_post_n = np.array(data_likelihoods_lt) * np.array(hs_likelihoods_lt) / \
                            (np.array(data_likelihoods_ll) * np.array(hs_likelihoods_ll) + np.array(data_likelihoods_lt) * np.array(hs_likelihoods_lt))

                    # joint_data_hs_ll_post_n = data_ll_post_n * hs_ll_post_n
                    # joint_data_hs_lt_post_n = data_lt_post_n * hs_lt_post_n
                    joint_data_hs_ll_post_sum = 1 + np.nansum(joint_data_hs_ll_post_n)
                    joint_data_hs_lt_post_sum = 1 + np.nansum(joint_data_hs_lt_post_n)
                else:
                    # data_ll_post_sum = 1
                    # data_lt_post_sum = 1
                    # hs_ll_post_sum = 1
            ...
            cond_ll = all([decision == 1 for decision in self.itv_decisions[-1:]])
            cond_lt = all([decision == 2 for decision in self.itv_decisions[-1:]])
...
def model_select_beta_credible(alpha, beta_, level=0.95, threshold=0.5):
