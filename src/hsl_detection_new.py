...
'''
This class is to detect anomalies using the residuals from LSTM and AR components.
'''
...
class hsl_detection:
    ...
    def __init__(
        ...
        self.residual_index = base_model.states_name.index("white noise")
        self.LTd_buffer = []
        self.p_anm_all = []
    ...
    def _create_drift_model(self, baseline_process_error_std):
        self.ar_component = self.generate_model.components["autoregression 2"]
    ...
    def filter(self, data, buffer_LTd: Optional[bool] = False):
        ...
        for i, (x, y) in enumerate(zip(data["x"], data["y"])):
            ...
            mu_obs_pred, var_obs_pred, mu_states_prior, var_states_prior = self.base_model.forward(x,
            ...
            _, _, mu_drift_states_posterior, var_drift_states_posterior = self.drift_model.backward(
                obs=self.base_model.mu_states_prior[self.residual_index] + lstm_mu_diff,
                obs_var=self.base_model.var_states_prior[self.residual_index, self.residual_index])
    ...
    def estimate_LTd_dist(self):
        ...
        for k in tqdm(range(len(generated_ts))):
            ...
            for i, (x, y) in enumerate(zip(time_covariate, generated_ts[k])):
                ...
                _, _, mu_states_prior, var_states_prior = base_model_copy.forward(x)
                (
                    _, _,
                    mu_states_posterior,
                    var_states_posterior,
                ...
                _, _, mu_drift_states_posterior, var_drift_states_posterior = drift_model_copy.backward(
                    obs=base_model_copy.mu_states_prior[self.residual_index] + lstm_mu_diff,
                    obs_var=base_model_copy.var_states_prior[self.residual_index, self.residual_index])
    ...
    def detect(
        ...
        while i < len(data["x"]):
            ...
            mu_obs_pred, var_obs_pred, mu_states_prior, var_states_prior = self.base_model.forward(data["x"][i])
    ...
    def collect_synthetic_samples(self, num_time_series: int = 10, save_to_path: Optional[str] = 'data/hsl_tsad_training_samples/hsl_tsad_train_samples.csv'):
        ...
        for k in tqdm(range(len(generated_ts))):
            ...
            for i, (x, y) in enumerate(zip(time_covariate, generated_ts[k])):
                ...
                #     base_model_copy.mu_states[self.residual_index] = drift_model_copy.mu_states[2]
