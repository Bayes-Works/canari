...
hsl_tsad_agent.mean_train, hsl_tsad_agent.std_train, hsl_tsad_agent.mean_target, hsl_tsad_agent.std_target = 0.00010628549, 0.0018827678, np.array([4.8311218e-04, 4.6687078e-02, 1.1108820e+02]), np.array([1.0975561e-02, 1.4099042e+00, 6.7753044e+01])
hsl_tsad_agent.learn_intervention(training_samples_path='data/hsl_tsad_training_samples/rsi_learn_samples_detrended_ts7.csv', 
                                  load_model_path='saved_params/NN_intervention_model_detrended_ts7_rsi.pkl', max_training_epoch=50)
...
for m in range(10):
    for n in tqdm(range(len(restored_data)//10)):
        ...
        mu_obs_preds, std_obs_preds, _, _ = hsl_tsad_agent.detect(test_data_k, apply_intervention=True)
...
results_df.to_csv("saved_results/prob_eva/detrend_ts7_results_rsi_lttoll.csv", index=False)