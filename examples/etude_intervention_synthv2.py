...
os.makedirs("LTU_Etude_intervention_synthv2", exist_ok=True)

LINE_WIDTH = 1
data_file = "/Users/michelwu/Desktop/Exponential component/donnes_synth_intervention_completev2.csv"
...
model.mu_states[1] += -model.mu_states[1]*0.2
model.mu_states[2] += model.mu_states[2]*1.55
# model.mu_states[3] = np.exp(-model.mu_states[0] + 0.5 * model.var_states[0, 0]) - 1
# model.mu_states[4] = model.mu_states[2] * model.mu_states[3] + model.var_states[2, 3]
# print(model.mu_states[2] * model.mu_states[3] + model.var_states[2, 3])
# model.mu_states[-1] += 4
model.mu_states[-1] += model.states.mu_posterior[-1][4] - ((np.exp(-model.mu_states[0] + 0.5 * model.var_states[0, 0]) - 1)* model.mu_states[2])
