...
# Normalize anm_mag
# anm_mag_norm = (anm_mag - data_processor.scale_const_mean[data_processor.output_col])/data_processor.scale_const_std[data_processor.output_col]
anm_mag_norm = anm_mag/data_processor.scale_const_std[data_processor.output_col]
# True baseline
LL_true = np.zeros_like(df_raw)
LT_true = np.zeros_like(df_raw)
for i in range(1, len(LL_true)):
    if i > time_anomaly:
        LL_true[i] = LL_true[i] + anm_mag_norm * (i-time_anomaly)
        LT_true[i] = LT_true[i] + anm_mag_norm
...
ax0.plot(time, LL_true, '--', color='blue')
...
ax1.plot(time, LT_true, '--', color='blue')
...
plt.savefig('delay_intervention.png', dpi=300)
plt.show()