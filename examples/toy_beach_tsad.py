...
beach = beach.to_frame(name='values')

# Convert year (fractional year) to datetime
def fractional_year_to_datetime(fractional_year):
    year = int(fractional_year)
    fraction = fractional_year - year
    
    # Determine the actual year length (for leap years)
    year_start = pd.Timestamp(year=year, month=1, day=1)
    year_end = pd.Timestamp(year=year + 1, month=1, day=1)
    year_length_days = (year_end - year_start).days
    
    # Compute elapsed days (including fractions for time of day)
    elapsed_days = fraction * year_length_days
    
    # Convert to timedelta to include time component
    dt = year_start + pd.to_timedelta(elapsed_days, unit='D')
    return dt

year = year.apply(fractional_year_to_datetime)
beach.index = year
beach.index.name = "date_time"
df_raw = beach.copy()
...
with open("saved_params/toy_beach_model_rebased.pkl", "rb") as f:
