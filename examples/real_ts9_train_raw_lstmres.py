...
op_model = Model(
    ...
    LocalTrend(mu_states=op_model_dict['states_optimal'].mu_prior[0][0:2].reshape(-1), var_states=[1e-12, 1e-12]),
    LSTM,
...
mu_ar = op_model.states.get_mean(states_type="posterior", states_name="autoregression", standardization=True)
std_ar = op_model.states.get_std(states_type="posterior", states_name="autoregression", standardization=True)

sum_noise_mu = lstm_residual + mu_ar
sum_noise_std = std_ar

# Plot the white noise states
fig, ax = plt.subplots(figsize=(10, 6))
ax.plot(sum_noise_mu, label="White noise mean")
ax.fill_between(
    np.arange(len(sum_noise_mu)),
    sum_noise_mu - sum_noise_std,
    sum_noise_mu + sum_noise_std,
    alpha=0.2,
...
sum_noise_obs = copy.deepcopy(train_data)
# Replayce sum_noise_obs['y'] with sum noise mu
sum_noise_obs['y'] = sum_noise_mu.reshape(-1, 1)
sum_noise_obs['y_var'] = sum_noise_std.reshape(-1, 1) ** 2

# Run ar_model on the sum noise states using filter
ar_model.filter(sum_noise_obs)
...
import pickle
with open("saved_params/real_ts9_tsmodel_raw_lstmres.pkl", "wb") as f:
