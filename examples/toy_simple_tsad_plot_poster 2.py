...
if (np.array(hsl_tsad_agent.p_anm_all) > 0.5).any():
    anm_detected_index = np.where(np.array(hsl_tsad_agent.p_anm_all) > 0.5)[0]
else:
...
base_model_mu_prior = np.array(hsl_tsad_agent.base_model.states.mu_prior[remove_until_index:])
base_model_var_prior = np.array(hsl_tsad_agent.base_model.states.var_prior[remove_until_index:])
...
p_anm_all = hsl_tsad_agent.p_anm_all[remove_until_index:]
