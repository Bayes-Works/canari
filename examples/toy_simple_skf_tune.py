...
def training(model, data_processor, num_epoch: int = 50):
    ...
    model.filter(data=data_processor.train_data)
    model.filter(data=data_processor.train_data)
    mu_validation_preds, std_validation_preds, _ = model.forecast(data_processor.validation_data)
    model.set_memory(states=model.states, time_step=0)

    states_optim = model.states
    mu_validation_preds_optim = np.array(mu_validation_preds).flatten()
    std_validation_preds_optim = np.array(std_validation_preds).flatten()
