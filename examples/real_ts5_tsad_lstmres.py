...
with open("saved_params/real_ts5_lstmres.pkl", "rb") as f:
...
base_model.lstm_net.load_state_dict(model_dict["lstm_network_params"])
gen_model.lstm_net.load_state_dict(model_dict["lstm_network_params"])

ltd_error = 1e-5

hsl_tsad_agent = hsl_detection(base_model=base_model, generate_model=gen_model, data_processor=data_processor, drift_model_process_error_std=ltd_error)

# Get flexible drift model from the beginning
hsl_tsad_agent_pre = hsl_detection(base_model=base_model.load_dict(base_model.get_dict()), generate_model=gen_model, data_processor=data_processor, drift_model_process_error_std=ltd_error)
...
# hsl_tsad_agent.mu_LTd = -5.148618372932382e-05 
# hsl_tsad_agent.LTd_std = 0.002965839040922683
hsl_tsad_agent.LTd_pdf = common.gaussian_pdf(mu = hsl_tsad_agent.mu_LTd, std = hsl_tsad_agent.LTd_std * 1)

# hsl_tsad_agent.collect_synthetic_samples(num_time_series=1000, save_to_path='data/hsl_tsad_training_samples/itv_learn_samples_real_ts5_lstmres.csv')

hsl_tsad_agent.nn_train_with = 'tagiv'
# hsl_tsad_agent.mean_train, hsl_tsad_agent.std_train, hsl_tsad_agent.mean_target, hsl_tsad_agent.std_target = 0.00041751217, 0.010767282, np.array([-1.6299923e-04, -1.7913265e-02, 1.0715303e+02]), np.array([1.0785332e-02, 1.3382866e+00, 6.2506252e+01])
...
hsl_tsad_agent.learn_intervention(training_samples_path='data/hsl_tsad_training_samples/itv_learn_samples_real_ts5_lstmres.csv',
                                  load_model_path='saved_params/NN_detection_model_real_ts5_lstmres.pkl', max_training_epoch=50)
