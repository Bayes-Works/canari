...
os.makedirs("LTU_Etude_intervention_synth", exist_ok=True)

LINE_WIDTH = 1
data_file = "/Users/michelwu/Desktop/Exponential component/donnes_synth_intervention_complete.csv"
...
# model.var_states[2, 2] += (model.mu_states[2].item() * 0.5) ** 2
# model.var_states[-1, -1] += (model.mu_states[-1].item() * 0.015) ** 2
# model.mu_states[0] = model.mu_states[0] * 0.55
# model.mu_states[1] = 0.00095
# model.mu_states[2] = 37.5
# # model.mu_states[3] = np.exp(-model.mu_states[0] + 0.5 * model.var_states[0, 0]) - 1
# # model.mu_states[4] = model.mu_states[2] * model.mu_states[3] + model.var_states[2, 3]
# model.mu_states[-1] += -1.9
...
### Cas avec des pourcentages pour essayer de trouver des cas convaincant
model.var_states[0, 0] += (model.mu_states[0].item() * 0.35) ** 2
model.var_states[1, 1] += (model.mu_states[1].item() * 0.05) ** 2
model.var_states[2, 2] += (model.mu_states[2].item() * 0.5) ** 2
model.var_states[-1, -1] += (model.mu_states[-1].item() * 0.015) ** 2
model.mu_states[0] = model.mu_states[0] * 0.55
model.mu_states[1] += -model.mu_states[1]*0.35
model.mu_states[2] += model.mu_states[2]*0.6
# model.mu_states[3] = np.exp(-model.mu_states[0] + 0.5 * model.var_states[0, 0]) - 1
# model.mu_states[4] = model.mu_states[2] * model.mu_states[3] + model.var_states[2, 3]

model.mu_states[-1] += model.states.mu_posterior[-1][4] - ((np.exp(-model.mu_states[0] + 0.5 * model.var_states[0, 0]) - 1)* model.mu_states[2]) -1
# model.var_states[0, 0] += (model.mu_states[0].item() * 0.20) ** 2
# model.var_states[1, 1] += (model.mu_states[1].item() * 0.6) ** 2
# model.var_states[2, 2] += (model.mu_states[2].item() * 0.8) ** 2
# model.var_states[-1, -1] += (model.mu_states[-1].item() * 0.015) ** 2
