...
# Resample the synthetic observations to an explicit hourly frequency.
...
# Load mean parameter values only for the lstm
global_param_path = "saved_params/global_model_param.bin"
model.load_lstm_parameter_means(global_param_path)

model.auto_initialize_baseline_states(train_data["y"][0:24*3])

# PyTAGI needs the number of forward samples even for a non-smoothed LSTM.
model.lstm_net.num_samples = len(train_data["y"]) + len(validation_data["y"])
if model.lstm_net.smooth:
    model.lstm_net.num_samples += model.lstm_net.lstm_infer_len

# Training
for epoch in range(num_epoch):

    # set white noise decay
    model.white_noise_decay(epoch, white_noise_max_std=sigma_v, white_noise_decay_factor=1.0) # white_noise_decay is generally not needed
    ...
    # Early-stopping
    model.early_stopping(evaluate_metric=mse, current_epoch=epoch, max_epoch=num_epoch, skip_epoch=0) # should not skip epochs
