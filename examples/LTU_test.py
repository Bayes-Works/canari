...
# model=Model(local_trend,ar,periodic)
model=Model(local_acceleration,ar,periodic)
...
plt.fill_between(
    df_part2.index,
    model.states.get_mean("level", states_type="posterior")
    + model.states.get_std("level", states_type="posterior"),
    model.states.get_mean("level", states_type="posterior")
    - model.states.get_std("level", states_type="posterior"),
    color="orange",
    alpha=0.3,
...
# y_observed = all_data2["y"].flatten()
