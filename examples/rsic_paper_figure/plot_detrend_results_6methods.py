...
df_lstmed = pd.read_csv("saved_results/prob_eva/detrended_allts_results_lstmed.csv")
df_tranad = pd.read_csv("saved_results/prob_eva/detrended_allts_results_tranad.csv")
...
df_tranad["anomaly_magnitude"] = np.abs(df_tranad["anomaly_magnitude"]) * 52
...
df_tranad["detection_rate"] = df_tranad["detection_time"].apply(
    lambda x: 0 if x >= 52 * 3 else 1
)
...
df_tranad["anomaly_detected_index"] = df_tranad["anomaly_detected_index"].apply(ast.literal_eval)
df_tranad["alarms_num"] = df_tranad["anomaly_detected_index"].apply(
    lambda x: len(x) if len(x) > 0 else 0
)
...
df_tranad.loc[df_tranad["detection_rate"] == 0, "alarms_num"] = 0
...
df_tranad_mean = df_tranad.groupby("anomaly_magnitude").agg(
    {
        "detection_time": ["mean", "std"],
        "detection_rate": ["mean", "std"],
        "alarms_num": ["mean", "std"],
...
ax[0].plot(df_skf_whitenoise_mean.index, df_skf_whitenoise_mean["detection_time"]["mean"], label="Matrix Profile", color = "tab:red")
ax[0].fill_between(
...
ax[0].plot(df_prophet_mean.index, df_prophet_mean["detection_time"]["mean"], label="Prophet", color = "tab:purple")
ax[0].fill_between(
...
ax[0].plot(df_lstmed_mean.index, df_lstmed_mean["detection_time"]["mean"], label="LSTMED", color = "tab:brown")
ax[0].fill_between(
...
ax[0].plot(df_tranad_mean.index, df_tranad_mean["detection_time"]["mean"], label="TranAD", color = "tab:pink")
ax[0].fill_between(
    df_tranad_mean.index,
    df_tranad_mean["detection_time"]["mean"] - df_tranad_mean["detection_time"]["std"],
    df_tranad_mean["detection_time"]["mean"] + df_tranad_mean["detection_time"]["std"],
    alpha=0.2,
    color = "tab:pink"
)
...
ax[1].plot(df_skf_mean.index, df_skf_mean["detection_rate"]["mean"], label="SKF", color = "tab:green")
ax[1].plot(df_skf_whitenoise_mean.index, df_skf_whitenoise_mean["detection_rate"]["mean"], label="Matrix profile", color = "tab:red")
ax[1].plot(df_prophet_mean.index, df_prophet_mean["detection_rate"]["mean"], label="Prophet", color = "tab:purple")
ax[1].plot(df_lstmed_mean.index, df_lstmed_mean["detection_rate"]["mean"], label="LSTMED", color = "tab:brown")
...
ax[2].plot(df_skf_mean.index, df_skf_mean["alarms_num"]["mean"], label="SKF", color = "tab:green")
ax[2].fill_between(
    df_skf_mean.index,
...
ax[2].plot(df_skf_whitenoise_mean.index, df_skf_whitenoise_mean["alarms_num"]["mean"], label="Matrix profile", color = "tab:red")
ax[2].fill_between(
    df_skf_whitenoise_mean.index,
...
ax[2].plot(df_prophet_mean.index, df_prophet_mean["alarms_num"]["mean"], label="Prophet", color = "tab:purple")
ax[2].fill_between(
    df_prophet_mean.index,
...
ax[2].plot(df_tranad_mean.index, df_tranad_mean["alarms_num"]["mean"], label="Catch", color = "tab:pink")
ax[2].fill_between(
    df_tranad_mean.index,
    df_tranad_mean["alarms_num"]["mean"] - df_tranad_mean["alarms_num"]["std"],
    df_tranad_mean["alarms_num"]["mean"] + df_tranad_mean["alarms_num"]["std"],
    alpha=0.2,
    color = "tab:pink"
)
ax[2].plot(df_il_mean.index, df_il_mean["alarms_num"]["mean"], label="IL", color = "tab:blue", linewidth=1.8)
ax[2].fill_between(
    df_il_mean.index,
